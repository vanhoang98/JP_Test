...
@section('content')
    ...
    <div class="row">
        @include("messages")
        <form action="{{ route("question_training.post_edit", $question->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="col-md-9">
                <div class="form-group">
                    <label for="">Trình độ</label>
                    <select name="level" id="" class="form-control">
                        <option value="1" @if($question->level == 1) selected @endif>N1</option>
                        <option value="2"  @if($question->level == 2) selected @endif>N2</option>
                        <option value="3"  @if($question->level == 3) selected @endif>N3</option>
                        <option value="4"  @if($question->level == 4) selected @endif>N4</option>
                        <option value="5"  @if($question->level == 5) selected @endif>N5</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="cate_tests" id="" class="form-control select2">
                        @foreach($cate_tests as $item)
                            <option value="{{$item->id}}" @if( $question->cate_test_id == $item->id ) selected @endif>
                                {{$item->name}}                                
                            </option>
                        @endforeach
                    </select>
